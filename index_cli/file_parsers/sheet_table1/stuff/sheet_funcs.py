...
def get_value(sh, row, col):
    val, _ = get_value_type(sh, row, col)
    return val
...
def get_date(date_str, exp=False):
    date = 0
    if isinstance(date_str, (float, int)):
        try:
            ...
            date = int(time.mktime(tuple(date_list)))
    ...
    elif isinstance(date_str, string_types):
        try:
